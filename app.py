...
from summarizer import build_summary_with_source
...
def main() -> None:
    ...
    if st.button("Check invoices", type="primary"):
        ...
        summary_text, summary_source = build_summary_with_source(result_df)
        if summary_source == "groq":
            st.caption("Summary source: 🤖 Groq LLM")
        else:
            st.caption("Summary source: 🧩 Rule-based template")
